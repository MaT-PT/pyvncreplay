...
def get_rle_length(data: IO[bytes]) -> int:
    ...
    while True:
        b = read_byte(data)
        if b < 0:
            raise ValueError("Unexpected end of data while decoding RLE")
        length += b
        if b < 0xFF:
            break
    return length
...
def decode_zrle_tile(
    ...
    if subencoding < 0:
        raise ValueError("Unexpected end of data while decoding ZRLE tile")

    if subencoding == 0:  # Raw
        pixdata += data.read(width * height * cpixel_size)
        if len(pixdata) < width * height * cpixel_size:
            raise ValueError("Not enough data to decode ZRLE tile")
        ...
        raise ValueError(f"Unsupported ZRLE subencoding: {subencoding}")
    ...
    if len(pixdata) > total_size:
        ...
        raise ValueError("ZRLE tile data is shorter than expected")
    return pixdata
